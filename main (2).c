...
int main() {
    ...
    char products[PRODUCT_COUNT][20];

    int menu, product, subMenu, q;

    for (int i = 0; i < PRODUCT_COUNT; i++) {
        strcpy(products[i], "미입력");
    }
    ...
    while (1) {
        ...
        switch (menu) {
            ...
            case 2:
                ...
                if (subMenu == 1) {
                    for (int i = 0; i < PRODUCT_COUNT; i++) {
                        scanf("%d", &q);
                        if (q > inventory[i]) {
                            printf("상품 %d(%s) 재고 부족! 현재 재고: %d\n", i + 1, products[i], inventory[i]);
                            q = inventory[i];
                        }
                        sale[i] += q;
                        inventory[i] -= q;
            ...
            case 3: {
                ...
                printf("가장 많이 판매된 상품 : ID %d, 상품명: %s, 판매량 %d\n", maxID + 1, products[maxID], sale[maxID]);
                printf("가장 적게 판매된 상품 : ID %d, 상품명: %s, 판매량 %d\n", minID + 1, products[minID], sale[minID]);
